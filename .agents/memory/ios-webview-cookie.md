...
`SameSite=None` lifts the site restriction so the cookie is sent in all contexts. It requires `Secure: true`. The production HTTPS proxy terminates TLS, and `trust proxy: 1` is set in `app.ts` so Express sees HTTPS from the `X-Forwarded-Proto` header — meaning `secure: true` works correctly in production.

**How to apply:** In `artifacts/api-server/src/app.ts`, the express-session cookie block:
```js
cookie: {
  httpOnly: true,
   secure: true,           // always — production proxy uses HTTPS
