...
description: Required production configuration for running the API on Railway.
---
...
**How to apply:** Provision Railway Postgres and set `DATABASE_URL`, `SESSION_SECRET`, `OPENAI_API_KEY`, `APP_BASE_URL`, `RESEND_API_KEY`, and `REVENUECAT_WEBHOOK_SECRET` as applicable. Let Railway inject `PORT`.