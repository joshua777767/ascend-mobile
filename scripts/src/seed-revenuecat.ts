...
const PROJECT_NAME = "Ascend";

const PRODUCT_IDENTIFIER = "ascend_pro_monthly";
const PLAY_STORE_PRODUCT_IDENTIFIER = "ascend_pro_monthly:monthly";

const PRODUCT_DISPLAY_NAME = "Ascend Pro Monthly";
const PRODUCT_USER_FACING_TITLE = "Ascend Pro Monthly";
const PRODUCT_DURATION = "P1M";

const APP_STORE_APP_NAME = "Ascend";
const APP_STORE_BUNDLE_ID = "com.ascend.fitness";
const PLAY_STORE_APP_NAME = "Ascend";
const PLAY_STORE_PACKAGE_NAME = "com.ascend.fitness";

const ENTITLEMENT_IDENTIFIER = "pro";
const ENTITLEMENT_DISPLAY_NAME = "Ascend Pro";

const OFFERING_IDENTIFIER = "default";
const OFFERING_DISPLAY_NAME = "Default";
...
async function seedRevenueCat() {
  ...
  console.log("Test Store API Key:", testStoreApiKeys?.items.map((i) => i.key).join(", ") ?? "N/A");
  console.log("App Store API Key:", appStoreApiKeys?.items.map((i) => i.key).join(", ") ?? "N/A");
  console.log("Play Store API Key:", playStoreApiKeys?.items.map((i) => i.key).join(", ") ?? "N/A");
  console.log("====================\n");
  ...
  if (testStoreApiKeys?.items[0]) {
    console.log(`REVENUECAT_TEST_API_KEY=${testStoreApiKeys.items[0].key}`);
  }
  if (appStoreApiKeys?.items[0]) {
    console.log(`REVENUECAT_IOS_API_KEY=${appStoreApiKeys.items[0].key}`);
  }
  if (playStoreApiKeys?.items[0]) {
    console.log(`REVENUECAT_ANDROID_API_KEY=${playStoreApiKeys.items[0].key}`);
  }
}
