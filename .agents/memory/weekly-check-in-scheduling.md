...
Weekly goal check-ins must be completion-driven: the first due time is account creation plus seven days, and every later due time is seven days after the latest completed check-in row. A missed due date stays due until completion; dismissing the modal must not advance the schedule. The separate weekly review prompt also needs the authenticated account creation timestamp as its first-run anchor; a missing localStorage timestamp must not mean immediately due.
